...
def get_network(args, net, use_gpu=True, gpu_device = 0, distribution = None):
    if net == 'sam_adaptered':
...
def random_click(mask, point_labels = 1, inout = 1):
    ...
    if len(indices)>0:
        XY=indices[np.random.randint(len(indices))]
    else:
        mask[0,0]=1
        indices = np.argwhere(mask == inout)
        XY=indices[np.random.randint(len(indices))]
